...
CREATE TABLE IF NOT EXISTS `mydb`.`media` (
  ...
  `employe_reference` INT NOT NULL,
  PRIMARY KEY (`chemin`),
  INDEX `fk_media_employe1_idx` (`employe_reference` ASC) VISIBLE,
  CONSTRAINT `fk_media_employe1`
    FOREIGN KEY (`employe_reference`)
    REFERENCES `mydb`.`employe` (`reference`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Table `mydb`.`employe_has_centre_interet`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `mydb`.`employe_has_centre_interet` (
  `employe_reference` INT NOT NULL,
  `centre_interet_id` INT NOT NULL,
  PRIMARY KEY (`employe_reference`, `centre_interet_id`),
  INDEX `fk_employe_has_centre_interet_centre_interet1_idx` (`centre_interet_id` ASC) VISIBLE,
  INDEX `fk_employe_has_centre_interet_employe1_idx` (`employe_reference` ASC) VISIBLE,
  CONSTRAINT `fk_employe_has_centre_interet_employe1`
    FOREIGN KEY (`employe_reference`)
    REFERENCES `mydb`.`employe` (`reference`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_employe_has_centre_interet_centre_interet1`
